...
class BVHSystem:
    ...
    def __init__(self, max_nodes=200000):
        ...
        self.stack = ti.field(ti.i32, shape=64)
        # 图元索引列表（用于叶子节点）
    ...
    def intersect_bvh(self, ray_origin, ray_dir, t_min, t_max, primitive_system, node_index):
        ...
        front_face = True
        # 使用堆栈进行迭代遍历（避免递归）
        self.stack[0] = node_index
        stack_size = 1
        while stack_size > 0:
            stack_size -= 1
            current_node_idx = self.stack[stack_size]
            ...
            if self.ray_aabb_intersect(ray_origin, ray_dir, node.bbox_min, node.bbox_max, t_min, closest_t):
                if node.primitive_count > 0:
                    ...
                    if node.right_child != -1:
                        self.stack[stack_size] = node.right_child
                        stack_size += 1
                    if node.left_child != -1:
                        self.stack[stack_size] = node.left_child
                        stack_size += 1
...
def create_cornell_box():
    ...
    scene_manager.add_cube(
        center=[-1, -half_size + cube_size/2, 1], size=cube_size, rotation_angles=[0, -10, 0], material_id=0
    )
    ...
    scene_manager.add_cuboid(
        ...
        rotation_angles=[60, 20, 0],  # 绕y轴逆时针旋转20度
